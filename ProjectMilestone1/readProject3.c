...
vector3_t initOrientation (void)
{
    ...
    //numbers = getAcclData();
    char    currentAccl[] = {0, 0, 0, 0, 0, 0, 0};
    ...
    {
    case PUSHED:
        currentAccl[0] = ACCL_DATA_X0;
        ref_value.x = (currentAccl[2] << 8) | currentAccl[1];
        ref_value.y = (currentAccl[4] << 8) | currentAccl[3];
        ref_value.z = (currentAccl[6] << 8) | currentAccl[5];

        //new_data.x = numbers.x - ref_x;
       // new_data.y = numbers.y - ref_y;
       // new_data.z = numbers.z - ref_z;
  //  }
   // if(start_flag == 0) {
    //    ref_x = ref_value.x;
   //     ref_y = ref_value.y;
...
getAcclData (void)
{
    ...
    vector3_t dddata;
    dddata.x = acceleration.x - ref_data.x;
    dddata.y = acceleration.y - ref_data.y;
    dddata.z = acceleration.z - ref_data.z;
...
main (void)
{
   ...
   // vector3_t display_value;
    ...
    {
        change_state();
        SysCtlDelay (SysCtlClockGet () / 130);    // Approx 2 Hz
